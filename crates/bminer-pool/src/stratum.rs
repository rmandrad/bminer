    ...
    fn parse_work(&self, params: &[Value]) -> Result<Work> {
        if params.len() < 9 {
            return Err(PoolError::ProtocolError(
                "Invalid mining.notify params".to_string(),
            ));
        }
        ...
        let merkle_branches: Vec<[u8; 32]> = params[4]
            .as_array()
...
impl PoolClient for StratumClient {
    ...
    async fn subscribe(&mut self) -> Result<()> {
        ...
        self.send_request("mining.subscribe", vec![json!("bminer/0.1.0")])
            .await?;
            ...
            if let Some(array) = result.as_array() {
                if array.len() >= 2 {
                    ...
                    if let Some(extranonce1_hex) = array[1].as_str() {
                        self.extranonce1 = hex::decode(extranonce1_hex).map_err(|e| {
                            PoolError::ProtocolError(format!("Invalid extranonce1: {}", e))
                        })?;
                    }
    ...
    async fn authorize(&mut self) -> Result<()> {
        ...
        self.send_request(
            "mining.authorize",
            vec![json!(worker_identity), json!(self.config.password)],
        )
        .await?;
...
mod tests {
    ...
    fn appends_worker_name_to_wallet_address() {
        let client = test_client("bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", "bminer");
