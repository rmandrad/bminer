...
fn default_threads_per_block() -> u32 { 256 }
fn default_blocks_per_grid() -> u32 { 1024 }
...
fn default_cpu_threshold() -> f32 { 20.0 }
fn default_min_idle() -> u64 { 60 }
fn default_check_interval() -> u64 { 5 }
fn default_gpu_threshold() -> u32 { 10 }
...
impl BminerConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.pool.url.is_empty() {
            return Err(ConfigError::ValidationError("Pool URL cannot be empty".to_string()));
        }
        
        // Validate pool username
        if self.pool.username.is_empty() {
            return Err(ConfigError::ValidationError("Pool username cannot be empty".to_string()));
        }
        ...
        if self.idle.cpu_threshold < 0.0 || self.idle.cpu_threshold > 100.0 {
            return Err(ConfigError::ValidationError("CPU threshold must be between 0 and 100".to_string()));
        }
