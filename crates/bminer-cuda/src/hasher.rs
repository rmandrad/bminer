...
impl CudaHasher {
    ...
    fn initialize_backend(device_id: usize) -> (ComputeBackend, String) {
        ...
        match Self::catch_probe_panic(|| Self::load_mining_kernel(&device)) {
            Ok(Ok(())) => match Self::allocate_gpu_buffers(&device) {
                Ok(buffers) => (
                    ComputeBackend::Cuda {
                        device,
            ...
            Err(panic) => {
                let reason = format!(
                    "failed to initialize CUDA kernel: {}",
                    Self::panic_message(panic)
                );
                let device_name = format!("{} [CPU fallback: {}]", gpu_name, reason);
                (ComputeBackend::CpuFallback, device_name)
    ...
    async fn mine_cpu(
        ...
        for i in 0..count {
            ...
            let block_header =
                BlockHeader::new(version, prev_block, merkle_root, timestamp, bits, nonce);
    ...
    fn mine_gpu(
        ...
        device
            ...
            .map_err(|e| {
                MiningError::GpuError(format!("Failed to copy block1 prefix to GPU: {}", e))
            })?;
        device
            .htod_sync_copy_into(&target_words, target_dev)
            .map_err(|e| MiningError::GpuError(format!("Failed to copy target to GPU: {}", e)))?;
        device.memset_zeros(found_count_dev).map_err(|e| {
            MiningError::GpuError(format!("Failed to reset GPU result counter: {}", e))
        })?;

        let kernel = device
            .get_func(CUDA_MODULE_NAME, CUDA_KERNEL_NAME)
            .ok_or_else(|| {
                MiningError::GpuError("CUDA mining kernel was not loaded".to_string())
            })?;
        ...
        unsafe {
            kernel
                ...
                .map_err(|e| {
                    MiningError::GpuError(format!("Failed to launch CUDA mining kernel: {}", e))
                })?;
        }
        ...
        device
            ...
            .map_err(|e| {
                MiningError::GpuError(format!("Failed to read GPU result counter: {}", e))
            })?;
        let found_count = found_count_host[0].min(MAX_GPU_RESULTS as u32) as usize;
...
impl Hasher for CudaHasher {
    async fn hash_range(
        ...
        match &self.backend {
            ComputeBackend::Cuda { device, buffers } => {
                match self.mine_gpu(device, buffers, work, start_nonce, count, extranonce2) {
                    Ok(results) => Ok(results),
                    Err(_) => self.mine_cpu(work, start_nonce, count, extranonce2).await,
                }
            }
            ComputeBackend::CpuFallback => {
                self.mine_cpu(work, start_nonce, count, extranonce2).await
            }
        }
    }
...
mod tests {
    ...
    fn midstate_path_matches_full_sha256d() {
        let header = BlockHeader::new(
            0x20000000, [0x11; 32], [0x22; 32], 0x05f5e100, 0x170d5c5a, 0x12345678,
        )
        .to_bytes();
